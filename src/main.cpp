...
// Copyright (c) 2013 The Landcoin developers
...
uint256 hashGenesisBlock("0x00000030f9d6323fcc48c587b206c49214828a0082cb1b9a3e80fa9874c6acc7");
static const unsigned int timeGenesisBlock = 1393959742;
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 LND if made of 50 LND inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
static const int64 nTargetTimespan = 2 * 60 * 60;
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
    ...
    while (nShift < 0)
    {
	 ...
	 //premined 0.5% for dev, support, and bounty etc
	if(nHeight > 15 && nHeight < 21)
	{
		nSubsidy = 11000 * COIN;
	}
...
*/
        ...
        // Genesis block
        const char* pszTimestamp = "Unix timestamp 1393959742";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0xcf875b16f73035fbf7a39501a98acf7be94fd4593b742fb8c8599d57ad2ed8ce"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LandcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LandcoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running LandcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("LandcoinMiner terminated\n");
        throw;
    }
}
